...
export function ChatBubble({ message, isLastMessage, streaming }: ChatBubbleProps) {
  ...
  function copyText() {
    // Build HTML with superscript citations + sources list
    ...
    if (message.sources?.length) {
      ...
      if (primary.length || expanded.length) {
        htmlText += '<hr style="margin:8px 0;border:none;border-top:1px solid #ccc"><p><b>Sources</b></p><ul>';
    ...
    // Copy as rich text via hidden element + selection (works in Zotero/Firefox)
