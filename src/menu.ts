...
function svgIcon(path: string): string {
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" width="16" height="16"><path fill="context-fill" fill-opacity="context-fill-opacity" d="${path}"/></svg>`;
...
const ICONS = {
  ...
  // Plugin brand icon (Phosphor Lightbulb)
...
const CONNECTION_REQUIRED_IDS = [
  'zotero-ai-library-chat',
  'zotero-ai-graph',
  'zotero-ai-discovery',
  'zotero-ai-health',
  'zotero-ai-queue',
...
export function setMenusConnected(win: Window, connected: boolean) {
  const doc = win.document;
  for (const id of CONNECTION_REQUIRED_IDS) {
...
export function registerMenus(win: Window) {
  ...
  menu.setAttribute('image', svgIcon(ICONS.lightbulb));
